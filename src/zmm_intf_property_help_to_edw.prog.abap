*&---------------------------------------------------------------------*
*& Report ZMM_INTF_PROPERTY_HELP_TO_EDW
*&---------------------------------------------------------------------*
*& KDURAI, 11/12/2023
*&---------------------------------------------------------------------*
REPORT zmm_intf_property_help_to_edw.
...
SELECTION-SCREEN BEGIN OF BLOCK b1 WITH FRAME TITLE TEXT-001.
  PARAMETERS : p_srv TYPE string LOWER CASE DEFAULT '/interfaces/edw/INT0053/out/'.
...
START-OF-SELECTION.
  ...
  CALL FUNCTION 'LVC_FIELDCATALOG_MERGE'
    EXPORTING
      i_structure_name       = 'ZPROPERTY_HELP'
    CHANGING
      ct_fieldcat            = gt_fcat
    EXCEPTIONS
...
* Implement suitable error handling here
  ...
  SELECT * FROM zproperty_help "#EC CI_NOWHERE
    INTO TABLE @DATA(lt_zproperty_help). "#EC CI_ALL_FIELDS_NEEDED

  DATA(lv_file_name) = 'INT0053_' && sy-datum && sy-uzeit && '.TXT'.
  ...
  LOOP AT lt_zproperty_help INTO DATA(ls_zproperty_help).
    CLEAR: gs_data,gv_data.
    LOOP AT gt_fcat INTO ls_fcat.
      lv_tabix = sy-tabix.
      UNASSIGN <lfs_field>.
      ASSIGN COMPONENT ls_fcat-fieldname OF STRUCTURE ls_zproperty_help TO <lfs_field>.
  ...
  IF 1 = 2.
    DATA: lt_data TYPE TABLE OF string,
          lv_file TYPE string VALUE '/interfaces/edw/INT0053/archive/'.
